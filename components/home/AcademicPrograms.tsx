...
export default function AcademicPrograms({ onOpenEnquiry, onOpenDetails }: AcademicProgramsProps) {
  ...
  const renderIcon = (type: string) => {
    switch (type) {
      ...
      case "elec":
        return (
          <svg className="w-7 h-7 text-sky-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <section className="py-16 px-4 bg-slate-50" id="programs">
      <div className="max-w-7xl mx-auto">
        <div className="text-center max-w-2xl mx-auto mb-10">
          ...
          <p className="text-slate-600 text-sm">
            BSIET offers both four-year B.Tech Degree programs affiliated to DBATU, Lonere, and three-year Polytechnic
            Diploma courses affiliated to MSBTE, Mumbai.
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-3 mb-10">
          ...
          >
            Polytechnic Diploma Programs (MSBTE)
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {programs.map((prog) => (
            <div
              key={prog.id}
              className="bg-white rounded-2xl p-6 border border-slate-200 hover:border-blue-500 shadow-sm hover:shadow-xl transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-[11px] font-mono font-bold bg-slate-100 text-slate-800 px-2.5 py-1 rounded border border-slate-200">
                    DTE: {prog.dteCode}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-3 rounded-xl border border-slate-100 mb-6">
                  <div>
                    <span className="text-slate-400 block text-[10px] uppercase">Affiliation</span>
                    <strong className="text-slate-800">{prog.affiliation}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px] uppercase">Intake</span>
                    <strong className="text-slate-800">{prog.intake} Seats</strong>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-slate-400 block text-[10px] uppercase">Eligibility</span>
                    <strong className="text-slate-800">{prog.eligibility}</strong>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-700 hover:from-blue-700 hover:to-indigo-800 text-white font-bold text-xs py-2.5 rounded-lg shadow transition-all cursor-pointer text-center"
                >
                  Apply Now
                </button>
                <button
                  onClick={() => onOpenDetails(prog)}
                  className="px-4 py-2.5 border border-slate-300 hover:border-slate-400 text-slate-700 hover:text-slate-900 font-semibold text-xs rounded-lg transition-all cursor-pointer"
                >
                  Details &raquo;
                </button>
              </div>
