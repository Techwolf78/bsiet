  ...
  const facilities = [
    {
      ...
      iconBg: "bg-blue-50/70 text-[#002b66] border-blue-100",
      icon: (
        <svg className="w-4 h-4 sm:w-5 sm:h-5 text-[#002b66]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-6">
          {facilities.map((fac, idx) => (
            <div
              key={idx}
              className="p-3.5 sm:p-6 rounded-xl sm:rounded-2xl border border-slate-200 bg-white hover:border-[#002b66] hover:shadow-md transition-all group flex items-start sm:flex-col justify-between gap-3 sm:gap-0"
            >
              <div className="flex items-start gap-3 sm:block">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 text-xs sm:text-base mb-0.5 sm:mb-2 group-hover:text-[#002b66] transition-colors">
                    {fac.title}
                  </h3>
